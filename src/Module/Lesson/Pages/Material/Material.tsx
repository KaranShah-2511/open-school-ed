...
function Material() {
  ...
  const [copyTmpLesson, setCopyTmpLesson] = useStateMounted<any>();
  ...
  useEffect(() => {
    (async () => {
      await dataService
        .get("LESSON_TMP")
        .then((tmpData) =>{
          setTmpLesson(tmpData);
          setCopyTmpLesson(tmpData);
        });
    })();
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, []);
  ...
  const getChannel = async (search) => {
    ...
    if (!tSearch.Searchby) {
      tSearch.Searchby = [copyTmpLesson.LessonTopic];
    }
    tSearch["SubjectBy"] = [copyTmpLesson.SubjectText.SubjectName];
  ...
  const handleInputChange = (event, channel) => {
    if (tmpLesson != undefined) {
      if (event.target.checked) {
        setTmpLesson((prevValues) => {
          return {
            ...prevValues,
  ...
  const result: any = (data) => {
    if (tmpLesson) {
      let confirmation = tmpLesson.ChannelList.find(({ id }) => id === data);
      ...
      <div className="video-gallery-main">
        ...
        ) : null}
        {
          <React.Fragment>
            <h3>
              Videos
              {tmpLesson && tmpLesson.ChannelList.length
                ? `(${tmpLesson.ChannelList.length})`
                : ""}
            </h3>
